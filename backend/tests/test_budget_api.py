...
def _plan(lo, hi=None, note=None):
    """Cuerpo del PUT. Con un solo valor arma una **banda de ancho cero**, que se
    comporta igual que el target de un solo número de antes: por eso los tests
    viejos siguen escritos así y valen como no-regresión del modelo de rango."""
    body = {"daily_min_usd": lo, "daily_max_usd": hi if hi is not None else lo}
    if note is not None:
        body["note"] = note
    return body
...
async def test_get_budget_serializes_money_as_string(app_client, monkeypatch):
    ...
    await app_client.put("/api/v1/budget/londres", json=_plan("10"), headers=h)
...
async def test_pre_trip_shows_plan(app_client, monkeypatch):
    ...
    await app_client.put("/api/v1/budget/londres", json=_plan("20"), headers=h)
    await app_client.put("/api/v1/budget/paris", json=_plan("30"), headers=h)
...
async def test_put_creates_and_updates(app_client, monkeypatch):
    ...
    r = await app_client.put("/api/v1/budget/londres",
                             json=_plan("20", note="hostel con cocina"), headers=h)
    assert r.status_code == 200
    assert r.json()["daily_max_usd"] == "20.00"
    assert r.json()["note"] == "hostel con cocina"

    r2 = await app_client.put("/api/v1/budget/londres", json=_plan("35"), headers=h)
    assert r2.status_code == 200
    assert r2.json()["daily_max_usd"] == "35.00"
...
async def test_put_unknown_slug_is_422(app_client, monkeypatch):
    h = await _seed_and_auth(app_client)
    r = await app_client.put("/api/v1/budget/narnia", json=_plan("20"), headers=h)
...
async def test_put_archived_slug_is_422(app_client, monkeypatch):
    ...
    h = await _seed_and_auth(app_client)
    r = await app_client.put("/api/v1/budget/vieja", json=_plan("20"), headers=h)
    assert r.status_code == 422
...
async def test_put_rejects_bad_amounts(app_client, monkeypatch):
    ...
    for bad in ("0", "-5", "10000"):
        r = await app_client.put("/api/v1/budget/londres", json=_plan(bad), headers=h)
...
async def test_budget_requires_auth(app_client):
    ...
    assert (await app_client.put("/api/v1/budget/londres", json=_plan("20"))).status_code == 401
...
async def test_band_is_serialized_with_both_edges(app_client, monkeypatch):
    h = await _seed_and_auth(app_client)
    _freeze_today(monkeypatch, date(2026, 8, 6))  # día 2 de Londres
    r = await app_client.put("/api/v1/budget/londres", json=_plan("4", "8"), headers=h)
    assert (r.json()["daily_min_usd"], r.json()["daily_max_usd"]) == ("4.00", "8.00")

    j = (await app_client.get("/api/v1/budget", headers=h)).json()
    c = _city(j, "londres")
    assert (c["target_min_usd"], c["target_max_usd"]) == ("4.00", "8.00")
    assert c["target_daily_usd"] == "6.00"        # el centro
    # Vivir de bruno en Londres = 15 en 2 noches = 7,50/día: adentro de la banda.
    assert c["band_position"] == "in"
    assert c["edge_delta_pct"] is None
    assert isinstance(c["delta_pct"], float)      # el desvío al centro sí existe

    p = j["projection"]
    assert (p["living_budget_min_usd"], p["living_budget_max_usd"]) == ("12.00", "24.00")
    assert p["living_budget_usd"] == "18.00"


async def test_band_position_over_and_under(app_client, monkeypatch):
    h = await _seed_and_auth(app_client)
    _freeze_today(monkeypatch, date(2026, 8, 6))  # 7,50/día de vivir

    await app_client.put("/api/v1/budget/londres", json=_plan("1", "5"), headers=h)
    c = _city((await app_client.get("/api/v1/budget", headers=h)).json(), "londres")
    assert c["band_position"] == "over"
    assert round(c["edge_delta_pct"], 1) == 50.0   # 7,50 contra un techo de 5

    await app_client.put("/api/v1/budget/londres", json=_plan("10", "15"), headers=h)
    c = _city((await app_client.get("/api/v1/budget", headers=h)).json(), "londres")
    assert c["band_position"] == "under"
    assert round(c["edge_delta_pct"], 1) == -25.0  # 7,50 contra un piso de 10


async def test_put_rejects_inverted_band(app_client, monkeypatch):
    """Una banda invertida no es un plan: budget.py la descartaría en silencio."""
    h = await _seed_and_auth(app_client)
    r = await app_client.put("/api/v1/budget/londres", json=_plan("50", "20"), headers=h)
    assert r.status_code == 422
    # Un piso igual al techo sí es válido: es el target de un solo número.
    assert (await app_client.put("/api/v1/budget/londres", json=_plan("20", "20"),
                                 headers=h)).status_code == 200


async def test_cushion_block(app_client, monkeypatch):
    h = await _seed_and_auth(app_client)
    _freeze_today(monkeypatch, date(2026, 8, 6))  # día 2 de 3 en Londres
    await app_client.put("/api/v1/budget/londres", json=_plan("10", "20"), headers=h)

    cu = (await app_client.get("/api/v1/budget", headers=h)).json()["cushion"]
    assert cu["covered_nights"] == 3
    assert cu["budget_to_date_usd"] == "30.00"     # centro 15 x 2 noches vividas
    assert cu["living_to_date_usd"] == "15.00"
    assert cu["cushion_usd"] == "15.00"
    assert cu["remaining_nights"] == 2             # 3 - 2 + 1 (hoy cuenta)
    assert cu["needed_daily_usd"] == "15.00"       # (45 - 15) / 2
    assert cu["avg_target_daily_usd"] == "15.00"


async def test_cushion_is_null_without_bands(app_client, monkeypatch):
    h = await _seed_and_auth(app_client)
    _freeze_today(monkeypatch, date(2026, 8, 6))
    cu = (await app_client.get("/api/v1/budget", headers=h)).json()["cushion"]
    assert cu["cushion_usd"] is None
    assert cu["needed_daily_usd"] is None
    assert cu["covered_nights"] == 0


async def test_current_category_mix(app_client, monkeypatch):
    """El desglose de vivir de la parada en curso, sin alojamiento ni generales."""
    h = await _seed_and_auth(app_client)
    _freeze_today(monkeypatch, date(2026, 8, 6))
    await app_client.put("/api/v1/budget/londres", json=_plan("10", "20"), headers=h)

    mix = (await app_client.get("/api/v1/budget", headers=h)).json()["current"]["by_category"]
    cats = (await app_client.get("/api/v1/categories", headers=h)).json()
    comida = next(c["id"] for c in cats if c["name"] == "Comida")

    assert [m["category_id"] for m in mix] == [comida]   # el alojamiento no es vivir
    assert mix[0]["living_usd"] == "15.00"
    assert mix[0]["share_pct"] == 100.0
    assert mix[0]["trip_share_pct"] == 100.0            # es el único vivir del viaje
    assert mix[0]["ratio"] == 1.0
