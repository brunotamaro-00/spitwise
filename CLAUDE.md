...
11. **Presupuesto de "vivir"** (`app/budget.py`, `StopBudget`): "vivir" = todo menos alojamiento y generales, y **no es un campo**: es el `other_usd`/`other_per_day_usd` que ya calcula `build_trip_pace` (alojamiento prorrateado afuera, generales excluidos por no tener `stop_slug`, todo en `user_share`). `budget.py` es puro y **consume la lista `cities` de `build_trip_pace`** — nunca re-agrega movimientos: duplicar el prorrateo o `user_share` es la forma garantizada de que `/ciudades` y `/presupuesto` se contradigan.
    ...
    - Un one-off caro imputado a una ciudad (vuelo, entrada) cuenta como vivir y hace picar ese día — es correcto, no un bug. El bot **lee** (`qa/tools.budget_status`) pero nunca escribe planes.
...
- **Presupuesto por chat** (`qa/tools.budget_status`): solo lectura. Responde "¿vamos bien en X?" contra el **rango** planeado de la parada, nunca contra el promedio del viaje, y una parada sin plan lo dice en vez de estimarlo. Devuelve `band` (`under`/`in`/`over`) más el colchón del viaje; el `note` de la tool explica que el techo es el límite y el centro el objetivo. Los planes se cargan en `/presupuesto` (deep link vía `copy.link_budget`). Está en `_EVIDENCE_TOOLS` de `bot/qa.py`: sin eso, un "USD 0" correcto apoyado en la tool caía en la guarda de falso cero.
