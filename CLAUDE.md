...
│   │   ├── qa/trip_tools.py  # tools del agente Q&A de viaje (guías/notas/documentos)
│   │   ├── whatsapp/         # Meta client, dedupe, signature
│   │   ├── categories/       # catálogo fijo de 11 categorías
│   │   ├── andiamo.py        # sync de stops
│   │   ├── andiamo_content.py # sync de guías + notas + documentos (Q&A de viaje)
│   │   ├── balance.py        # neto puro (sin I/O)
│   │   ├── budget.py         # rango de vivir vs gasto real (puro)
...
### Canal viaje (guías + notas + documentos)
...
Carga markdown real desde `../andiamo/content/guides` + notas y documentos
alineados al Itinerary. Hoy fijo `2026-09-25` (Viena). **16 escenarios**.
Transcript → `scripts/bot_trip_scenarios.md`.

Los escenarios comparten **una sola DB** (solo se resetean los historiales): un
check con conteos absolutos cuenta lo sembrado y lo de las conversaciones
previas. Scopealo por un marcador propio del escenario (ver `note_check`).
...
| **StopBudget** | Plan de gasto diario de "vivir" por parada: un **rango** `daily_min_usd`–`daily_max_usd` (USD/día **por persona**). Dato autoral, no viene de Andiamo — tabla propia sin FK porque el sync borra la fila de `Stop` de una parada sin movimientos (ver invariante 11) |
| **GuideDoc / StopGuide / TripNote / TripDocument / SyncMeta** | Cache del contenido de viaje de Andiamo (guías markdown, mapeo stop→guía, notas, **metadata** de documentos, version del export) para el Q&A de viaje del bot — sync en `app/andiamo_content.py`. `TripDocument` no guarda el archivo: el binario vive en R2 y lo sirve Andiamo en `/api/documents/<id>` |
...
11. **Presupuesto de "vivir"** (`app/budget.py`, `StopBudget`): "vivir" = todo menos alojamiento y generales, y **no es un campo**: es el `other_usd`/`other_per_day_usd` que ya calcula `build_trip_pace` (alojamiento prorrateado afuera, generales excluidos por no tener `stop_slug`, todo en `user_share`). `budget.py` es puro y **consume la lista `cities` de `build_trip_pace`** — nunca re-agrega movimientos: duplicar el prorrateo o `user_share` es la forma garantizada de que `/ciudades` y `/presupuesto` se contradigan.
    - **El plan es un rango, no un número** (`daily_min_usd`–`daily_max_usd`, USD/día por persona, uno por parada y compartido por los dos — los tramos que difieren ya son paradas distintas: Portugal vs Pititas). `PRESUPUESTO.md` siempre dio bandas y colapsarlas a un punto era precisión falsa. **El techo es el límite** (`band_position` = `over` recién arriba de `max`) y el **centro es el objetivo**: contra él se miden devengado, varianza, colchón, proyección y `remaining_daily`. Una banda de ancho cero (`min == max`) reproduce exactamente el modelo de target único de antes — ese es el test de no-regresión. `edge_delta_pct` mide contra el borde violado y es `None` adentro del rango: adentro no hay desvío que reportar, ese es el punto de tener rango.
    - Una parada sin banda **no se compara ni se extrapola**: baja la cobertura, y la página nunca muestra una varianza sin la línea de cobertura al lado. `in_itinerary` (de `analytics.py`) es la frontera que evita que las paradas del otro sumen noches fantasma al presupuesto del viaje.
    - **Colchón y ritmo necesario** (`budget.trip_cushion`): `cushion_usd` = devengado del plan hasta hoy − gastado (signo sin clampear), `needed_daily_usd` = a cuánto por día hay que ir en lo que queda. `remaining_nights` suma "hoy" **solo si hoy cae dentro de una parada cubierta** — en un día de traslado entre dos paradas, hoy no es ninguna de las dos.
    - Un one-off caro imputado a una ciudad (vuelo, entrada) cuenta como vivir y hace picar ese día — es correcto, no un bug. El presupuesto es **web-only**: el bot no lo lee ni lo escribe, deflecta a `/presupuesto`.
    - **El hero de la parada en curso es el POTE, no una tasa** (`budget.current_city_budget`, `lib/budget.stayEnvelope`, `components/budget/CurrentHero`): el número grande es `remaining_budget_usd` — lo que queda del pote de esta ciudad (`envelope_usd` = **centro** × noches; `envelope_max_usd` es el techo y solo se marca en la barra). Antes acá vivía `remaining_daily_usd` ("USD 80 por día hasta el check-out"): se divide por los días que faltan, así que un almuerzo caro lo movía 10%, el último día concentraba todo el remanente, y era la única de las tres cifras de la card que no era real (el hero decía 80, la barra 36/día, el plan 57–83). La tasa sigue existiendo, como lectura secundaria. **Una sola tasa alimenta las tres lecturas** (`pote = tasa × días restantes`, la sub-línea por día, y el presupuesto de hoy), así que cierran entre sí: gastar hoy baja un poco la tasa de los días que quedan, y eso es correcto — el pote es finito.
      - **Fila HOY**: `spent_today_usd` sale de `analytics.other_today_usd` (vivir con `created_at` en la tz del viaje == hoy, agregado **en el loop de `build_trip_pace`** — abrir un segundo recorrido de movimientos en `budget.py` es lo que la invariante prohíbe). `left_today_usd` = tasa − gastado hoy, y es `None` **con el pote ya vacío**: restarle el gasto del día a una tasa negativa daba "gastaste 82, te pasaste 187", que mezcla el exceso de toda la parada con el del día. Sin banda el gasto de hoy igual se muestra: es un hecho, no una comparación.
      - **Un solo permiso en la página, y es el hero.** El strip de `CushionCard` dice "Ritmo del resto del viaje", no "Podés gastar": dos permisos de scope distinto en la misma pantalla es la forma más rápida de que ninguno se crea.
      - **Las dos primeras cards están podadas a propósito** (la demo pública la abre gente que entra de cero, y el hero llegó a pedir cuatro conceptos). El hero dice el pote, el plazo en días de la parada, cuánto va gastado y el día de hoy — se fueron la banda por día (57–83) y el plan devengado hasta hoy, que necesitaban su propia leyenda; el veredicto de ritmo viaja en **una palabra** en el `BandBadge`. Por eso `StayEnvelopeBar` es una barra que se llena y nada más: sin poste ni techo, con eje `max(pote, gastado)` — así el final de la barra **es** el número que el pie escribe al lado (con el techo como eje terminaba en una cifra que no estaba escrita en ningún lado). No usa `BAR_MAX_USD`: ese eje fijo es de las barras de USD/día. Pasados de pote **no** pinta todo el relleno de rojo: el tramo que entraba en el plan queda blanco y solo el exceso va en la rampa, que es la parte accionable. `CushionCard` sigue la misma regla — un número, una barra sin rótulos (la resta ya está dibujada) y **una frase** con el ritmo del resto del viaje, que antes era un segundo display con su conteo de noches y el plan promedio al lado.
      - Cambiar la forma de lo cacheado obliga a subir `CACHE_SCHEMA_VERSION` (`lib/queryClient.ts`): la caché persistida de 7 días hidrata el payload viejo y la página **crashea** hasta el refetch (pasó con el bloque `cost`).
    - **La barra tiene eje fijo** (`lib/budget.BAR_MAX_USD` = USD 120/día por persona) y el color sale de **una** función, `bandLevel` → rampa `--color-heat-*` (teal ahorrando · verde en plan · dorado al límite · naranja pasado · rojo pasado fuerte). Escalar cada barra contra su propio plan —lo que hacía antes— dejaba a Londres (plan 68–95, real 81) y a una parada de plan 20–30 con el relleno a la misma altura: prolijo y mudo. Fuera del eje el relleno capea y lo dice con el tope (`overCap`), nunca creciendo. La rampa **sí llega al rojo**, revirtiendo el "nunca rojo" viejo de `Badge`/`BandBadge`: pasarse fuerte es lo único de esta página que tiene que verse de lejos. La zona del plan va en tinte neutro para no competir con la rampa, y el color nunca viaja solo (siempre el `BandBadge` con la palabra + el `aria-label` con los tres números).
    - **"En qué se te va" dice las dos cosas**: proporción (`ratio` = share de la parada / share del viaje, base **comprometida**) y magnitud (`per_day_usd`/`delta_per_day_usd`, base **devengada** — `living_by_category_accrued` de `analytics.py`, sin paradas futuras). Unificar las bases rompe una de las dos: un prepago de la próxima ciudad es parte de la mezcla pero no de un promedio por día vivido. Canarios: la suma de `per_day_usd` da `other_per_day_usd` de la ciudad.
    - **El costo del viaje entero se compone, no se juzga** (`budget.trip_cost`, bloque `cost`, hero de `TripCostCard`): vivir proyectado + alojamiento (lo reservado **más** una estimación de las noches que faltan a `per_night_usd`) + generales comprometidos. Los generales **no se extrapolan** (los vuelos y pases del tramo que falta ya están comprados; espeja `analytics._project`) y sin una sola noche reservada `lodging_estimated_usd` es `None`: no se inventa un precio por noche. `basis` (`projected` | `committed`) es el interruptor del copy — sin ninguna ciudad cerrada no hay ritmo y el total es "lo comprometido hasta hoy", no una proyección; el frontend no lo deduce de un `null` suelto (`lib/budget.tripCostRead`). Las noches sin reservar salen de `fixed.total_nights` y **no** de `projection.budget_nights` (universos distintos: uno son los días del itinerario, el otro las noches elegibles). El **veredicto contra la banda sigue midiendo solo vivir** y por eso la barra vive abajo del divisor con el eyebrow "el plan mide solo vivir": dormir y los generales son plata comprometida, y pintar el total de rojo contra una banda que no lo mide es un veredicto inventado. **Hay un solo total del viaje en la app**: la card (`components/budget/TripCostCard`) es la misma en el Dashboard y al pie de `/presupuesto`, con la misma query key `["budget"]`; la diferencia es que `/presupuesto` le cuelga abajo el veredicto de vivir (`ProjectionCard` la envuelve pasándoselo como `children`) y el Dashboard no. Reemplazó a la card "Ritmo del viaje", que mostraba `trip.projected_total_usd` (`analytics._project`: ritmo all-in × noches) e **ignoraba las reservas futuras ya cargadas**: dos totales calculados distinto en dos pantallas era la forma garantizada de que la app se contradiga a sí misma. El campo sigue en el payload de `/dashboard/pace` pero ya no lo renderiza nadie.
...
| Budget | `GET /budget` (análisis completo: ciudad en curso, colchón, filas por ciudad, proyección, fijos), `PUT /budget/{slug}` (upsert de la banda `daily_min_usd`/`daily_max_usd`; 422 si el slug no es una parada real o si el máximo es menor que el mínimo), `DELETE /budget/{slug}` |
...
| Integration | `GET /cities/spend`, `GET /cities/spend-detail?slug=`, `GET /trip/spend`, `POST /andiamo/sync-hook` (todos `X-Api-Key`; body opcional `{event}`: `stops.changed` default, `notes.changed`, `documents.changed`, `guides.changed`), `POST /andiamo/sync` (JWT) |
...
- **Presupuesto: fuera del bot, deflecta a la app**. No hay tool de presupuesto (`budget_status` se eliminó). "¿Vamos bien en X?" sigue ruteando a `question` (no a `trip_question`: ahí caería en la guía `presupuesto.md` de Andiamo, peor), y el `_SYSTEM` del Q&A financiero tiene una regla que lo manda a `/presupuesto` con el deep link de `copy.link_budget()` — nunca lo estima ni lo deriva del promedio del viaje, que es justo el número equivocado (el plan es un rango por parada). `_EVIDENCE_TOOLS` vuelve a ser solo `aggregate_expenses`/`list_movements`. Escenario `presupuesto-deflect`.
  - **El borde que hay que cuidar**: se deflecta compararse contra el PLAN, no lo YA GASTADO. "¿Cuánto gastamos por día?" / "promedio por día" / "cuánto llevamos gastado" son datos del ledger y se responden con `aggregate_expenses` + `get_itinerary` como siempre. Una regla de deflección redactada de más se los come — el canario es el escenario `promedio-dia`.
- **Q&A de viaje** (`bot/trip_qa.py` + `qa/trip_tools.py`): canal AISLADO del financiero — prompt, tools (search/list/read de guías + notas + `search_documents`) e historial propios. Grounded: solo responde con lo que hay en `guide_docs`/`trip_notes`/`trip_documents`; si no está, lo dice. Respuestas cortas + deep-link `{andiamo_url}/guias/<guide>/<doc>`. Preguntas de plata las deriva al canal financiero. No mezclar tools/prompt entre los dos agentes.
  - **Documento ≠ guía**: "¿dónde está el voucher del hostel?" pide un ARCHIVO (`search_documents` → `{andiamo_url}/api/documents/<id>` + la parada), no lo que dice la guía. El link pide sesión abierta en Andiamo (no es signed URL): es el caso normal en el celu. Por eso el parser manda a `trip_question` las preguntas por un documento aunque digan "entrada" o "pasaje" — palabras que si no leen como gasto.
- **Notas dictadas** (`bot/trip_notes.py` + `andiamo_notes.py`): intent `trip_note` del parser → preview + botones `note_save:`/`note_cancel:` (pending `note_create`) → `POST {ANDIAMO_URL}/api/integration/notes`. La parada sale de `capture.resolve_place` (mismas reglas que un gasto). Un reintento inmediato; si falla dos veces el pending queda ABIERTO para re-tocar Guardar. Spitwise **nunca** escribe `trip_notes`: esa tabla es cache del sync, la fuente de verdad es Andiamo. Borde caro: una nota con un monto adentro ("anotá que cobra 20 de depósito") NO es un gasto — escenario `nota-con-plata`.
...
- Split por defecto shared; cambio de split vía NL (`edit`). Los botones legacy `split_*` se eliminaron.
...
| Q&A de viaje (guías/notas/documentos) | `backend/app/bot/trip_qa.py` + `backend/app/qa/trip_tools.py` |
| Notas dictadas por chat | `backend/app/bot/trip_notes.py` + `backend/app/andiamo_notes.py` |
| Sync contenido Andiamo | `backend/app/andiamo_content.py` |
| Frontend routes | `frontend/src/App.tsx` |
| Design tokens | `frontend/src/index.css` |
| Deploy | `DEPLOY.md` |
| Presupuesto (rango de vivir) | `backend/app/budget.py` + `backend/app/api/budget.py` |
...
- `living_by_category` (por ciudad y del viaje) también sale de `build_trip_pace`, del mismo loop que parte alojamiento vs resto: **la suma del dict tiene que dar `other_usd`**. Calcularlo aparte en `budget.py` sería la forma de que el desglose de "En qué se te va" no cierre con el total de la ciudad. Canario: `test_living_by_category_sums_to_other`.
...
- Sync de contenido (guías/notas/documentos, `andiamo_content.py`): **NUNCA** colgarlo del path caliente del webhook — solo lifespan, sync-hook y lazy dentro de `handle_trip_question` (dispara task, no bloquea). Andiamo lo alimenta con `GET /api/guides/export` (bulk con `version`; no-op si no cambió), `GET /api/notes` y `GET /api/integration/documents`; sus mutaciones pingean `notes.changed` / `documents.changed`. Payload inválido nunca arrasa el snapshot. **`notes_only=True` significa "saltear las guías"**, no "solo notas": sincroniza notas *y* documentos, que son metadata chica y no justifican un tercer flag de dirty. Notas y documentos comparten la asimetría con las guías: lista vacía **sí** vacía la tabla (borrar el último es legítimo).
