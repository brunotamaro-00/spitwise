...
11. **Presupuesto de "vivir"** (`app/budget.py`, `StopBudget`): "vivir" = todo menos alojamiento y generales, y **no es un campo**: es el `other_usd`/`other_per_day_usd` que ya calcula `build_trip_pace` (alojamiento prorrateado afuera, generales excluidos por no tener `stop_slug`, todo en `user_share`). `budget.py` es puro y **consume la lista `cities` de `build_trip_pace`** — nunca re-agrega movimientos: duplicar el prorrateo o `user_share` es la forma garantizada de que `/ciudades` y `/presupuesto` se contradigan.
    ...
    - **El hero de la parada en curso es el POTE, no una tasa** (`budget.current_city_budget`, `lib/budget.stayEnvelope`, `components/budget/CurrentHero`): el número grande es `remaining_budget_usd` — lo que queda del pote de esta ciudad (`envelope_usd` = **centro** × noches; `envelope_max_usd` es el techo y solo se marca en la barra). Antes acá vivía `remaining_daily_usd` ("USD 80 por día hasta el check-out"): se divide por los días que faltan, así que un almuerzo caro lo movía 10%, el último día concentraba todo el remanente, y era la única de las tres cifras de la card que no era real (el hero decía 80, la barra 36/día, el plan 57–83). La tasa sigue existiendo, como lectura secundaria. **Una sola tasa alimenta las dos lecturas** (`pote = tasa × días restantes` y la sub-línea por día), así que cierran entre sí: gastar hoy baja un poco la tasa de los días que quedan, y eso es correcto — el pote es finito. **Hubo una fila HOY** (gasto del día contra su tasa, alimentada por un `other_today_usd` en `build_trip_pace`) y se sacó entera, backend incluido: era un cuarto número en la card focal y no cambiaba ninguna decisión.
